...
curl -v \
  ...
  -X POST \
  -d '{"phone": "59179716902@c.us", "content": "test message"}' \
  http://localhost:3000/message/owen
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: *app_name
          image: owencio/whatsapp-app:1.8.4
...
```
docker build -t owencio/whatsapp-app:1.8.4 .
docker push owencio/whatsapp-app:1.8.4
```
